...
export function CreateWalletDialog({ open, onOpenChange, onSuccess }: CreateWalletDialogProps) {
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      ...
      toast({
        title: t('wallets.messages.createSuccess'),
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>{t('wallets.actions.create')}</DialogTitle>
          <DialogDescription>
            Create a new wallet. All fields marked with * are required.
          </DialogDescription>
        </DialogHeader>
