...
export function DeleteTransactionDialog({ open, onClose, transaction, onSuccess }: DeleteTransactionDialogProps) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (
        errorMessage.includes("TRANSACTION_DELETE_NEGATIVE_BALANCE") ||
    ...
    <AlertDialog open={open} onOpenChange={onClose}>
      <AlertDialogContent className="max-w-md">
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>{t("wallet.transaction.deleteConfirmMessage")}</AlertDialogDescription>
        </AlertDialogHeader>
        ...
        {isCompleted && (
          <Alert variant="destructive" className="border-destructive/50 bg-destructive/10">
            ...
            <AlertDescription>{t("wallet.transaction.completedWarning")}</AlertDescription>
          </Alert>
        )}
        ...
        <div className="space-y-3 py-4 border-y">
          ...
          <div className="flex items-center justify-between">
            ...
            <Badge variant={getTypeVariant(transaction.type)}>{t(`wallet.transaction.type.${transaction.type}`)}</Badge>
          </div>
          <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="text-sm">{format(new Date(transaction.createdAt), "MMM dd, yyyy HH:mm")}</span>
          </div>
        </div>
